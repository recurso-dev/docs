...
| 500 | `internal_error` | The referenced customer, plan, or coupon could not be resolved — a nonexistent or malformed `customer_id`/`plan_id` currently surfaces as 500. | Verify the customer and plan exist in your tenant and the coupon code is valid before creating. |
