...
| 500 | `internal_error` | The subscription or the target plan does not exist in your tenant — a nonexistent or malformed `plan_id` currently surfaces as 500. | Verify both IDs; preview the change first with `GET /v1/subscriptions/{id}/preview-change?plan_id=...`. |
