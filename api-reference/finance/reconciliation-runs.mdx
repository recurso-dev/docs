...
<Info>
  Only the **summary** is stored, not the individual discrepancies — the row
  records that a check happened and its headline result. To see the current
  drift in detail, run the report again. Correcting drift stays a deliberate
