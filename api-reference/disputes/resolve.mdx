...
|--------|------|------|-----|
| `400` | `validation_failed` | `id` in the path is not a UUID; the body is not valid JSON; `outcome` is not `accept` or `reject`; `credit_amount` is negative; or `credit_amount` exceeds the invoice total (the dispute stays `open`) | Fix the offending field |
...
| `500` | `internal_error` | Credit issuance failed — the disputed invoice could not be loaded, or credit issuance is not enabled on this deployment. The dispute stays `open`. | Retry, or resolve without `issue_credit` |
