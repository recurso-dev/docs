...
| `page` | integer | No | Page number (default `1`). Ignored when `offset` is given. |
| `per_page` | integer | No | Rows per page (default `50`, capped at `250`). `limit` is accepted as an alias and wins when both are sent. |
| `offset` | integer | No | Row offset (default `0`). Overrides the offset computed from `page`. |
