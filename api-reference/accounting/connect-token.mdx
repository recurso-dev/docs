...
tooling reads. If a connection for the provider already exists — active,
soft-disconnected by
[Disconnect Accounting Integration](/api-reference/accounting/disconnect), or
deactivated after a token-refresh failure — it is updated in place and
reactivated with `sync_status` reset to `idle` and `last_error` cleared;
otherwise a new one is created. Trigger a push afterwards with
[Trigger Accounting Sync](/api-reference/accounting/sync).
