...
The `GET /v1/finance/revrec/report` endpoint returns a tenant-level deferred-revenue
rollforward for a **single month**: what was recognized that month, the deferred
balance still on the books, when that balance is scheduled to release, and how it
splits by currency. It takes required `month` (1–12) and `year` query parameters —
not a date range:
...
| Field | Description |
|-------|-------------|
| `month` / `year` | The period the report covers |
| `recognized_amount` | Revenue recognized during `month`/`year` (minor units) |
| `deferred_balance` | All still-pending recognition on the books (minor units, summed across currencies) |
| `upcoming` | The deferred balance grouped by the month it is scheduled to recognize (`month`, `year`, `amount`) |
| `by_currency` | The deferred balance split by schedule currency (`currency`, `deferred`) |

<Note>
  The report is a tenant-wide rollforward, not a per-subscription breakdown, and
  it does not take a date range — call it once per month you want to inspect. For
  a full recognized-plus-scheduled curve, use the revenue waterfall.
</Note>
...
<Tip>
  For a month's recognized total, prefer `GET /v1/finance/revrec/report` (above) —
  it returns `recognized_amount` for a `month`/`year` directly, so you don't have
...
takes a `month`/`year` and returns that month's `recognized_amount` directly. If
you'd rather sum ledger postings, list **Recognized Revenue** (`4100`) — the entries endpoint has
