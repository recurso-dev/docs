...
keywords: ["stripe", "razorpay", "gocardless", "gateway routing", "BYO gateway"]
---
...
- **GoCardless** — mandate-first bank debit (SEPA, Bacs): the European/UK
  ...
  asynchronous settlement via webhook). Configure `GOCARDLESS_ACCESS_TOKEN`
  (+ `GOCARDLESS_ENV=sandbox` for testing) and `GOCARDLESS_WEBHOOK_SECRET`,
  or let tenants connect their own account from **Integrations → Payment
  gateways** (token-only; test mode pairs with sandbox tokens). See the
  [setup guide](/setup/payment-gateway#bank-debit-mandates-gocardless).
- **Adyen** — **experimental** (API-verified against documented request
  shapes; sandbox certification in progress).
  Global card & wallet processing via Checkout Sessions, with off-session
  charges on stored payment methods. Configure `ADYEN_API_KEY`,
  `ADYEN_MERCHANT_ACCOUNT` (+ `ADYEN_ENV=test`, or `ADYEN_LIVE_URL_PREFIX`
  for live).
