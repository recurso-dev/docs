...
<Info>
  This page explains the concepts. To turn it on step by step, follow the [accounting-sync setup guide](/setup/accounting).
</Info>
...
There are no `accounting.*` webhook event types — monitor sync state with
[the sync-status endpoint](/api-reference/accounting/sync-status) (connection,
