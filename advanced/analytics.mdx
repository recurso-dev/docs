...
// Response
{
  ...
  query: "SELECT c.id, c.name, s.mrr, s.canceled_at FROM subscriptions s JOIN customers c ON s.customer_id = c.id WHERE s.status = 'canceled' AND s.canceled_at BETWEEN '2026-05-01' AND '2026-05-31'"
}
```
