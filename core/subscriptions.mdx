...
keywords: ["recurring billing", "subscription lifecycle", "trial", "cancel"]
---
...
// The first invoice is issued when the subscription activates.
// Fetch it, then collect on the hosted checkout page (gateway
// chosen by invoice currency) — or let a saved mandate autopay it:
const { data: invoices } = await recurso.invoices.list({
  subscription_id: subscription.id,
});
const payLink = `https://api.recurso.dev/checkout/${invoices[0].id}`;
```
...
subscription = create_subscription.sync(
    ...
    body=CreateSubscriptionRequest(
        customer_id=UUID("9f2c1e84-6a3d-4b7e-9c21-8f4a5d6e7b01"),
        plan_id=UUID("3e7a9c52-1b4d-4f8e-a6c3-2d5b8e9f0a14"),
    ),
)

# The first invoice is issued when the subscription activates.
invoices = list_invoices.sync(client=client, subscription_id=subscription.id)
pay_link = f"https://api.recurso.dev/checkout/{invoices.data[0].id}"
```

```go Go
subscription, _ := client.Subscriptions.Create(ctx, &recurso.SubscriptionCreateParams{
  CustomerID: "9f2c1e84-6a3d-4b7e-9c21-8f4a5d6e7b01",
  PlanID:     "3e7a9c52-1b4d-4f8e-a6c3-2d5b8e9f0a14",
})

// The first invoice is issued when the subscription activates.
invoices, _ := client.Invoices.List(ctx, &recurso.InvoiceListParams{
  SubscriptionID: subscription.ID,
})
payLink := "https://api.recurso.dev/checkout/" + invoices[0].ID
```
</CodeGroup>

There is no gateway parameter here: Recurso routes each payment to the right
gateway from the invoice currency and your [gateway connections](/advanced/payment-gateways),
and payment happens per invoice — on the [hosted checkout page](/core/payments)
or automatically via a saved mandate.
...
const subscription = await recurso.subscriptions.create({
  customer_id: '9f2c1e84-6a3d-4b7e-9c21-8f4a5d6e7b01',
  plan_id: '3e7a9c52-1b4d-4f8e-a6c3-2d5b8e9f0a14',
  coupon_code: 'LAUNCH20'  // 20% off first 3 months
});
```
...
<Tip>
Want to see the exact number for your own prices? The
[**Proration**](/core/proration) page has a live calculator that runs the same
math the API uses, plus how to preview any plan change with a single API call.
</Tip>
...
```python Python
from recurso.api.subscriptions import reactivate_subscription

# id is the subscription's UUID
result = reactivate_subscription.sync(subscription.id, client=client)

# cancel_at_period_end is cleared; billing resumes at next renewal
```

```go Go
result, _ := client.Subscriptions.Reactivate(ctx, "sub_xyz")
...
// Mandates are their own resource — register one against the
// subscription and renewals collect automatically:
const mandate = await recurso.mandates.create({
  customer_id: subscription.customer_id,
  subscription_id: subscription.id,
  vpa: 'customer@upi',
  max_amount: 999900,        // per-debit cap, minor units (₹9,999.00)
  frequency: 'monthly'
});
```
...
<CardGroup cols={2}>
  <Card title="Invoices" href="/core/invoices">
    What every billing period produces
  </Card>
  <Card title="Usage-based billing" href="/advanced/usage-billing">
